...
const uint8_t hwaddr_gbl[6] = { 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6 };

START_TEST(test_hwaddr_init)
{
	uint8_t hwaddr[6];
	int i;

	hwaddr_init(hwaddr, 0xff);
	for (i = 0; i < 6; i++)
		ck_assert_int_eq(hwaddr[i], 0xff);
}
END_TEST

START_TEST(test_hwaddr_eq)
{
	const uint8_t hwaddr[6] = { 0xf1, 0xf2, 0xf3, 0xf5, 0xf5, 0xf6 };

	ck_assert_int_eq(hwaddr_eq(hwaddr_gbl, hwaddr_gbl), true);
	ck_assert_int_eq(hwaddr_eq(hwaddr_gbl, hwaddr), false);
}
END_TEST

START_TEST(test_hwaddr_cp)
{
	uint8_t hwaddr[6];
	int i;
...
START_TEST(test_addr_to_str)
{
	...
	ether_addr_to_str(hwaddr_gbl, str, sizeof(str));
...
START_TEST(test_str_to_addr)
{
	...
	ck_assert_int_eq(hwaddr_eq(hwaddr_tmp, hwaddr_gbl), true);
...
START_TEST(test_dev_alloc_hwaddr)
{
	...
	dev = ether_dev_alloc(hwaddr_gbl);
	ck_assert(dev != NULL);

	ck_assert_int_eq(hwaddr_eq(dev->hwaddr, hwaddr_gbl), true);
...
{
	const uint8_t hwaddr_src[6] = { 0x1f, 0x2f, 0x3f, 0x4f, 0x5f, 0x6f };
	const uint8_t *hwaddr_dst = hwaddr_gbl;
...
Suite *ether_suite(void)
{
	...
	tcase_add_test(tc_misc, test_hwaddr_init);
	tcase_add_test(tc_misc, test_hwaddr_cp);
	tcase_add_test(tc_misc, test_hwaddr_eq);
